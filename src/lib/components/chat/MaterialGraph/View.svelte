	...
	>
		<header class="shrink-0 border-b border-gray-100 px-4 py-3 dark:border-gray-800">
			<div class="flex items-center justify-between gap-3">
				...
				<div class="text-right text-xs tabular-nums text-gray-500">
					<div>{Math.round((snapshot?.elapsed_ms ?? 0) / 100) / 10}s</div>
					<div>{snapshot?.evidence_count ?? 0} 条证据</div>
				</div>
			</div>
		</header>
		...
		>
			{#if selectedNode}<div class="mb-3">
					...
					<p class="mt-1 text-xs leading-5 text-gray-500">
						{selectedNode.description ?? selectedNode.summary ?? selectedNode.status}
					</p>
				</div>{/if}
			<div class="space-y-2" aria-label="执行日志">
				{#each (snapshot?.logs ?? []).slice().reverse() as log}<div class="flex gap-2 text-xs">
						<span class="mt-1.5 size-1.5 shrink-0 rounded-full bg-gray-400"></span>
						<div class="min-w-0">
							<div class="break-words text-gray-700 dark:text-gray-300">{log.message}</div>
							<div class="mt-0.5 text-[10px] text-gray-400">
								{log.node_id}{log.timestamp ? ` · ${log.timestamp}` : ''}
							</div>
						</div>
					</div>{:else}<p class="text-xs text-gray-400">节点日志会随工作流执行实时出现。</p>{/each}
			</div>
		</div>
