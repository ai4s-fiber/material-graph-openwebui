<script lang="ts">
	...
	$: duration =
		data?.duration ??
		(data?.duration_ms != null ? `${Math.round(Number(data.duration_ms))} ms` : '');
</script>
...
<div
	...
	class="graph-node w-[220px] rounded-xl border bg-white px-3 py-2.5 shadow-sm dark:bg-gray-900"
>
	...
	<div class="flex items-start justify-between gap-2">
		<div class="min-w-0">
			...
			<div class="mt-1 line-clamp-2 text-[11px] leading-4 text-gray-500 dark:text-gray-400">
				{data?.description ?? data?.summary ?? status}
			</div>
		</div>
		<span class="status-dot mt-1 size-2 shrink-0 rounded-full" aria-label={status}></span>
	</div>
...
<style>
	...
	:global(.dark) .graph-node {
		border-color: rgb(55 65 81);
	}
	.status-dot {
		background: rgb(156 163 175);
	}
	.graph-node.complete .status-dot {
